...
class AppModel {
    ...
    var currentState: String = Status.AWAINING_START.name

    private var field: Array<ByteArray> = array2dOfByte(
        FieldConstats.ROW_COUNT.value,
        FieldConstats.COLUMN_COUNT.value
    )

    enum class Status {
        AWAINING_START, ACTIVE, INACTIVE, OVER
    }
    ...
    fun isGameAwaitingStart(): Boolean{
        return currentState == Status.AWAINING_START.name
    }

    private fun booostScore(){
        score += 10
    ...
    private fun generateNextBlock(){
        currentBlock = Block.creatBlock()
    }

    private fun validTranslatio(position: Point, shape: Array<ByteArray>): Boolean{
        return if (position.y < 0 || position.x < 0){
            false
        }else if(position.y +shape.size > FieldConstats.ROW_COUNT.value){
            false
        } else if(position.x + shape[0].size > FieldConstats.COLUMN_COUNT.value){
            false
        } else {
    ...
    private fun moveValid(position: Point, frameNumber: Int?) : Boolean{
        ...
        return validTranslatio(position, shape as Array<ByteArray>)
    }
    ...
    private fun resetField(ephemeralCellsOnly: Boolean = true){
        for(i in 0 until FieldConstats.ROW_COUNT.value){
            (0 until FieldConstats.COLUMN_COUNT.value).filter { !ephemeralCellsOnly || field[i][it] == CellConstants.EPHEMERAL.value }
    ...
    fun endGame(){
        score = 0
        currentState = AppModel.Status.OVER.name
    }

    private fun resetModel(){
        resetField(false)
        currentState = Status.AWAINING_START.name
        score = 0
    }
}
